...
// =============================================
// PANTALLA DE CARGA + ARRANQUE
// reintenta hasta que el backend responda
// =============================================
...
function ocultarPantallaCarga() {
  ...
  setTimeout(function() { pantalla.style.display = "none"; }, 400);
}

function actualizarMensajeCarga(texto) {
  document.getElementById("mensajeCarga").textContent = texto;
}
...
async function arrancarApp() {
  ...
  var intentos = 0;
  var maxIntentos = 20; // espera hasta ~60 segundos

  while (intentos < maxIntentos) {
    try {
      actualizarMensajeCarga(intentos == 0 ? "Conectando con el servidor..." : "Esperando al servidor... (" + intentos + "/" + maxIntentos + ")");

      var res = await fetch("http://localhost:3000/api/clientes", {
        headers: { "Authorization": "Bearer " + token }
      });

      // si el servidor responde bien ya podemos continuar
      if (res.ok) {
        actualizarMensajeCarga("Cargando datos...");

        // cargar todo en paralelo
        await Promise.all([
          cargarClientesInterno(),
          new Promise(function(resolve) { cargarDatosConCallback("", "", 1, "", resolve); }),
          new Promise(function(resolve) { cargarEstadisticasConCallback("", "", "", resolve); })
        ]);
      ...
      // servidor aun no disponible, esperar y reintentar
    }
  ...
  // si llega aqui es que no pudo conectar tras todos los intentos
  actualizarMensajeCarga("❌ No se pudo conectar con el servidor. Recarga la página.");
}

// version de cargarClientes que devuelve promesa
function cargarClientesInterno() {
...
// version de cargarDatos con callback para usar en Promise
function cargarDatosConCallback(origen, salida, pagina, codigoBarras, callback) {
  var limite = document.getElementById("limitePagina").value;
  var url = "http://localhost:3000/api/paquetes?pagina=" + pagina + "&limite=" + limite;
  if (origen != "")     { url += "&origen=" + origen; }
  if (salida != "")     { url += "&salida=" + salida; }
  if (codigoBarras != "" && codigoBarras != undefined) { url += "&codigoBarras=" + codigoBarras; }

  fetch(url, { headers: { "Authorization": "Bearer " + token } })
  .then(function(res) { return res.json(); })
  .then(function(datos) {
    renderizarTabla(datos, pagina);
    if (callback) { callback(); }
  })
  .catch(function() { if (callback) { callback(); } });
}

// version de cargarEstadisticas con callback
function cargarEstadisticasConCallback(origen, salida, codigoBarras, callback) {
  var salidaSelect  = document.getElementById("filtroSalida");
  var salidaId      = salidaSelect.value;
  var nombreCliente = salidaId != "" ? salidaSelect.options[salidaSelect.selectedIndex].textContent : "";
  ...
  Promise.all([
    fetch(urlCompleta,  { headers: { "Authorization": "Bearer " + token } }).then(function(r) { return r.json(); }),
    fetch(urlSinOrigen, { headers: { "Authorization": "Bearer " + token } }).then(function(r) { return r.json(); })
  ])
  .then(function(res) {
    hacerGraficos(nombreCliente, res[0], res[1]);
    if (callback) { callback(); }
  })
  .catch(function() { if (callback) { callback(); } });
}

// extraer la parte de renderizado de tabla para reutilizarla
function renderizarTabla(datos, pagina) {
  ...
  for (var i = 0; i < datos.datos.length; i++) {
    ...
    var fecha = new Date(p.fechaSalida).toLocaleDateString("es-ES");
    html += "<tr>";
    html += "<td style='color:var(--text-muted);font-family:Space Mono,monospace;font-size:0.75rem;'>#" + p.id + "</td>";
    html += "<td class='columna-barcode'><a href='/paquete.html?codigo=" + encodeURIComponent(p.codigoBarras) + "' style='font-family:Space Mono,monospace;color:var(--accent);font-size:0.78rem;text-decoration:none;border-bottom:1px dashed var(--accent);'>" + p.codigoBarras + "</a></td>";
    html += "<td class='columna-fecha' style='color:var(--text-muted);'>" + fecha + "</td>";
    html += "<td><span class='etiqueta-verde'>" + p.origen + "</span></td>";
    html += "<td><span class='etiqueta-morada'>" + p.nombreCliente + "</span></td>";
    html += "<td style='text-align:right;white-space:nowrap;'>";
    html += "<button class='boton-icono' title='Editar' onclick='abrirModalEditar(" + JSON.stringify(p).replace(/'/g, "\\'") + ")'>✏️</button>";
    html += "<button class='boton-icono' title='Eliminar' onclick='abrirModalEliminar(" + p.id + ",\"" + p.codigoBarras + "\")'>🗑️</button>";
    html += "</td></tr>";
  }
